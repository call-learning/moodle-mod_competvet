 ...
 */
abstract class base_entity_selector extends base {
    /**
     * Setup form
     *
    ...
    public function get_sql_filter(array $values): array {
        ...
        $ids = $values["{$this->name}_values"] ?? [];
        if (empty($ids)) {
            return ['', []];
        }

        $paramprefix = database::generate_param_name() . '_';
        [$entitywhere, $entitparams] = $DB->get_in_or_equal($ids, SQL_PARAMS_NAMED, $paramprefix);

        return ["{$fieldsql} $entitywhere", array_merge($params, $entitparams)];
    }
