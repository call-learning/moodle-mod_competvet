# CompetVet local Sass build

This folder contains the module-local Grunt entrypoint used when building the module on its own.
...
The module uses a local `.stylelintignore` so the generated `styles.css` is not linted as source.
