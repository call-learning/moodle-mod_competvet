...
/**
   ...
   */Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.init=void 0,_notification=_interopRequireDefault(_notification),_modal_factory=_interopRequireDefault(_modal_factory),_modal_events=_interopRequireDefault(_modal_events),_ajax=_interopRequireDefault(_ajax);_exports.init=modulename=>{const selectedElements=(0,_generic_form_helper.getSelectedElement)("delete");selectedElements&&selectedElements.forEach((element=>{element.addEventListener("click",(event=>{event.preventDefault(),(0,_str.get_string)("observation:delete",modulename).then((title=>{_modal_factory.default.create({title:title,body:(0,_str.get_string)("observation:delete:confirm",modulename),type:_modal_factory.default.types.SAVE_CANCEL,large:!0}).then((modal=>(modal.getRoot().on(_modal_events.default.save,(()=>{_ajax.default.call([{methodname:"mod_competvet_eval_delete_observation",args:{observationid:event.target.dataset.id},done:data=>{data.success?(modal.hide(),window.location.assign(event.target.dataset.returnurl),window.location.reload()):_notification.default.exception({message:data.error})},fail:_notification.default.exception}])})),modal.show(),modal))).catch(_notification.default.exception)}))}))}))}}));
