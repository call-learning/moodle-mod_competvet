...
/**
   ...
   */_competstate=_interopRequireDefault(_competstate),_notification=_interopRequireDefault(_notification),_templates=_interopRequireDefault(_templates),_chartjs=_interopRequireDefault(_chartjs);const gradingApp=document.querySelector('[data-region="grading-app"]'),transformContext=(context,autoeval)=>{const self=_competstate.default.getValue("user").id,data=context["evaluation-results"],labels=data.evaluations.map((criterion=>criterion.criterion.label)),graders=[],colors=["rgba(255, 99, 132, 0.6)","rgba(54, 162, 235, 0.6)","rgba(255, 206, 86, 0.6)","rgba(75, 192, 192, 0.6)","rgba(153, 102, 255, 0.6)","rgba(255, 159, 64, 0.6)"],backgroundColors=["rgba(255, 99, 132, 0.2)","rgba(54, 162, 235, 0.2)","rgba(255, 206, 86, 0.2)","rgba(75, 192, 192, 0.2)","rgba(153, 102, 255, 0.2)","rgba(255, 159, 64, 0.2)"];return data.evaluations.forEach((criterion=>{criterion.grades.forEach((grade=>{if(grade.graderinfo&&grade.graderinfo.id&&(!autoeval||grade.graderinfo.id===self)&&(autoeval||grade.graderinfo.id!==self)){if(!graders[grade.graderinfo.id]){const color=colors.shift(),backgroundColor=backgroundColors.shift();graders[grade.graderinfo.id]={label:grade.graderinfo.fullname,data:[],fill:!0,backgroundColor:backgroundColor,bordercolor:color,pointBackgroundColor:color,pointBorderColor:"rgba(255, 255, 255, 1)",pointHoverBackgroundColor:"rgba(255, 255, 255, 1)",pointHoverBorderColor:color}}graders[grade.graderinfo.id].data.push(grade.level)}}))})),{labels:labels,datasets:Object.values(graders)}},chartConfig=async autoeval=>{let title=await(0,_str.get_string)("supervisorchart","mod_competvet");return autoeval&&(title=await(0,_str.get_string)("selfevaluation","mod_competvet")),{type:"radar",options:{title:{display:!0,text:title,position:"bottom",font:{size:16},fullSize:!0},scales:{r:{ticks:{display:!0,min:0,max:100,stepSize:25,callback:function(value){return 25===value||50===value||75===value||100===value?value:""}},pointLabels:{callback:function(label){return label.length>10?label.substring(0,10)+"...":label}},angleLines:{display:!1},suggestedMin:0,suggestedMax:100}},plugins:{title:{display:!0,text:title,position:"bottom"},legend:{display:!0,position:"bottom",labels:{usePointStyle:!0,pointStyle:"circle"}}}}}};(()=>{const region=gradingApp.querySelector('[data-region="'.concat("evaluation-chart",'"]')),template="mod_competvet/grading/components/".concat("evaluation-chart");_competstate.default.subscribe("evaluation-chart",(async context=>{void 0!==context["evaluation-chart"]&&_templates.default.render(template,context).then((async html=>{region.innerHTML=html;const evalchart=document.getElementById("evaluation-chart"),config=await chartConfig();config.data=transformContext(context),config.data.datasets.length>0&&new _chartjs.default(evalchart,config);const autoeval=document.getElementById("auto-evaluation-chart"),autoevalConfig=await chartConfig(!0);autoevalConfig.data=transformContext(context,!0),autoevalConfig.data.datasets.length>0&&new _chartjs.default(autoeval,autoevalConfig),0===config.data.datasets.length&&0===autoevalConfig.data.datasets.length&&(region.innerHTML="")})).catch(_notification.default.exception)}))})()}));
