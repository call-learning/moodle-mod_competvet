 ...
 *
 * @module     mod_competvet/local/grading2/grading_app_comments
 ...
 */
const observations = () => {
    const regionName = 'evaluations-comments';
    const evaluationsComments = gradingApp.querySelector(`[data-region="${regionName}"]`);
    const template = `mod_competvet/grading2/evaluations/comments`;
    const regionRenderer = (context) => {
        if (context.evaluations === undefined) {
            return;
        }
        Templates.render(template, context.evaluations).then((html) => {
            evaluationsComments.innerHTML = html;
            return;
